...
import { ShoppingCartIcon, PlusIcon, EyeIcon, FileTextIcon, PrinterIcon, SearchIcon } from "lucide-react";
...
import { insertOrderSchema, type InsertOrder, type Brick, type Tractor } from "@shared/schema";
      ...
      <html>
        ...
        <body>
          ...
          <div class="details">
            ...
            <p><strong>Total Amount:</strong> PKR${parseFloat(order.totalAmount).toLocaleString()}</p>
            ...
            <div className="flex items-center space-x-3">
              ...
              <Dialog open={showAddDialog} onOpenChange={setShowAddDialog}>
                <DialogTrigger asChild>
                  <Button className="bg-brick-primary hover:bg-brick-secondary" data-testid="button-create-order">
                  ...
                  <Form {...form}>
                    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            ...
                            <FormControl>
                              <Textarea {...field} data-testid="input-delivery-address" />
                            </FormControl>
                            <FormMessage />
                          </FormItem>
                        )}
                                ...
                                <SelectContent>
                                  {bricks?.map((brick) => (
                                    <SelectItem key={brick.id} value={brick.id}>
                                      {brick.type} - PKR{parseFloat(brick.unitPrice).toFixed(2)}
                                    </SelectItem>
                                  ))}
                                </SelectContent>
                              </Select>
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            <FormLabel>Assigned Tractor</FormLabel>
                            <Select onValueChange={field.onChange} value={field.value || ""}>
                              <FormControl>
                              ...
                              <SelectContent>
                                {tractors?.filter(t => t.status === 'available').map((tractor) => (
                                  <SelectItem key={tractor.id} value={tractor.id}>
                                    {tractor.registrationNumber} - {tractor.model}
                                  </SelectItem>
                                ))}
                              </SelectContent>
                            </Select>
                        ...
                        <div className="p-4 bg-gray-50 rounded-lg">
                          ...
                          <div className="space-y-1 text-sm">
                            <div className="flex justify-between">
                              <span>Bricks ({quantity.toLocaleString()} units):</span>
                              <span>PKR{(parseFloat(selectedBrick.unitPrice) * quantity).toLocaleString()}</span>
                            </div>
                            <div className="flex justify-between">
                            ...
                            <div className="flex justify-between">
                              <span>Tax (18%):</span>
                              <span>PKR{((parseFloat(selectedBrick.unitPrice) * quantity + 3500) * 0.18).toLocaleString()}</span>
                            </div>
                            <div className="flex justify-between font-bold border-t pt-1">
                              <span>Total:</span>
                              <span>PKR{parseFloat(calculateTotal(selectedBrick, quantity)).toLocaleString()}</span>
                            </div>
                          </div>
                        </div>
                      )}
                      ...
                      <div className="flex justify-end space-x-2">
                        ...
                        <Button type="submit" className="bg-brick-primary hover:bg-brick-secondary" data-testid="button-save-order">
                          Create Order
                        </Button>
                      </div>
                    </form>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredOrders.map((order) => (
                  <tr key={order.id} data-testid={`order-row-${order.id}`}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div>
                        <div className="text-sm font-medium text-gray-900" data-testid={`customer-name-${order.id}`}>
                          {order.customerName}
                        </div>
                        {order.customerPhone && (
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div>
                        <div className="text-sm font-medium text-gray-900">
                          {bricks?.find(b => b.id === order.brickType)?.type || 'Unknown'}
                        </div>
